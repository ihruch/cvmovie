...
@Component({
  selector: 'app-ratio-icon',
  templateUrl: './ratio-icon.component.html',
  styleUrls: ['./ratio-icon.component.scss']
})
export class RatioIconComponent implements OnInit {

  @ViewChild('wrapper', { read: ElementRef }) public wrapper : ElementRef;
  @ViewChild('canvas', { read: ElementRef }) public canvas: ElementRef;
  @ViewChild('procent', { read: ElementRef })   public spanProcent: ElementRef;
 ...
 ngOnInit() {
    ...
    this.wrapper.nativeElement.style.width = this.sizeRatingIcon + 4 +  'px';
    this.wrapper.nativeElement.style.height = this.sizeRatingIcon + 4 + 'px';
